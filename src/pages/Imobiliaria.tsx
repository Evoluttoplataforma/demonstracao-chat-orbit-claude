...
import { getImobiliariaSessionVariant } from "@/components/chat/imobiliariaCopyVariants";
...
import {
  ...
  Instagram, MapPin, AlertTriangle, Home, Key,
} from "lucide-react";

/* ── FAQ Data ── */
const faqItems = [
  { q: "O que é o Orbit?", a: "O Orbit é uma plataforma que entrega dezenas de agentes de IA especializados para executar a gestão da sua imobiliária. Cada agente cuida de uma área — processos, pessoas, indicadores, treinamento e mais. Eles trabalham 24/7, integrados aos dados do seu negócio." },
  { q: "Funciona para imobiliárias pequenas?", a: "Sim. Funciona para imobiliárias de 3 a 200 pessoas. Na verdade, quanto menor a equipe, mais impacto os agentes geram — porque substituem funções que você não consegue contratar." },
  { q: "Quanto tempo leva para implementar?", a: "Em até 7 dias seu time de agentes está operando. A Olívia, nossa especialista em IA, analisa o contexto da sua imobiliária e configura os agentes ideais para sua operação." },
  ...
  { q: "A demonstração é gratuita?", a: "Sim. São 30 minutos para você conhecer seus agentes, entender como atuariam na sua imobiliária e ver resultados reais. Sem compromisso." },
  { q: "E se minha imobiliária já usa CRM ou ERP?", a: "Perfeito. O Orbit não substitui suas ferramentas — ele preenche a lacuna entre ter a ferramenta e realmente usar. Os agentes garantem que processos sejam seguidos, indicadores monitorados e equipe treinada." },
];

/* ── Pain Points ── */
const painPoints = [
  { bold: "Cada negócio depende de mim para fechar.", sub: "Se eu não acompanho, o corretor deixa passar." },
  { bold: "Perco clientes por falta de follow-up,", sub: "não por falta de imóveis." },
  { bold: "Minha equipe vive apagando incêndio", sub: "e eu fico preso no operacional." },
  { bold: "Contratei corretores bons, mas sem processo", sub: "cada um atende do seu jeito." },
  { bold: "Não consigo tirar férias", sub: "porque nada funciona sem mim." },
  { bold: "Já tentei planilha, CRM, consultor…", sub: "e nada se mantém funcionando." },
];

/* ── AI Agents ── */
const agents = [
  { icon: Target, name: "Ag. Estrategista", desc: "Constrói planejamento estratégico, SWOT, BSC, objetivos e planos de ação para a imobiliária", equiv: "Consultor de planejamento — R$30-80k/projeto" },
  { icon: FolderCog, name: "Ag. de Processos", desc: "Mapeia processos de captação, venda, locação e pós-venda, gera playbooks e padrões", equiv: "Analista de processos sênior — R$8-12k/mês" },
  { icon: Users, name: "Ag. de Pessoas", desc: "Descreve cargos, analisa desempenho de corretores, cria PDIs e acompanha evolução", equiv: "Analista de RH/DHO — R$6-10k/mês" },
  { icon: GraduationCap, name: "Ag. de Treinamento", desc: "Cria microlearning diário sobre técnicas de venda, atendimento e mercado via WhatsApp", equiv: "Coordenador de T&D — R$7-12k/mês" },
  { icon: BarChart3, name: "Ag. de Indicadores", desc: "Monitora KPIs de vendas, locação, captação, gera hipóteses de causa raiz", equiv: "Analista de BI — R$8-15k/mês" },
  ...
  { icon: Home, name: "Ag. de Carteira", desc: "Organiza carteira de imóveis, identifica oportunidades de captação e giro de estoque", equiv: "Analista de inteligência — R$8-12k/mês" },
  { icon: Briefcase, name: "Ag. de Oportunidades", desc: "Mapeia oportunidades de mercado, novos segmentos e parcerias", equiv: "Analista de mercado — R$8-12k/mês" },
  ...
  { icon: FileText, name: "Ag. de Documentos", desc: "Gera documentos corporativos, contratos e checklists estruturados", equiv: "Analista documental — R$5-8k/mês" },
  { icon: TrendingUp, name: "Ag. de Vendas", desc: "Estrutura funil de captação, cadências de follow-up e análise de pipeline", equiv: "Coord. comercial — R$8-15k/mês" },
...
const results = [
  ...
  { value: "-85%", label: "Erros", desc: "Agente de Processos no WhatsApp: procedimentos na palma da mão, erros eliminados" },
  { value: "7 dias", label: "Implementação", desc: "Do onboarding ao primeiro resultado em uma semana" },
];

/* ── Process Steps ── */
const processSteps = [
  { icon: Zap, number: "01", title: "Conecte sua imobiliária", desc: "Cadastre sua imobiliária em menos de 5 minutos. A Olívia analisa o contexto do seu negócio." },
  { icon: Brain, number: "02", title: "Seus agentes são configurados", desc: "Com base na análise, a Olívia ativa os agentes ideais para sua operação imobiliária." },
...
const Imobiliaria = () => {
  ...
  const variant = useMemo(() => getImobiliariaSessionVariant(), []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (lookupError) console.error("LP Imobiliária: Lookup error:", lookupError);

      let leadId: string | null = null;
      const resolvedCopyVariant = copyVariant || variant?.id || "IM-A";
      const leadPayload = {
        nome,
        sobrenome,
        ...
        cargo: "Dono / Sócio de Imobiliária",
        oque_faz: "Imobiliária",
        ...cleanUtm,
      } as any;
      ...
      if (existingLead) {
        ...
        if (updateError) console.error("LP Imobiliária: Erro ao atualizar lead:", updateError);
        leadId = existingLead.id;
      } else {
        const { data: inserted, error: insertError } = await supabase.from("leads")
          .insert(leadPayload)
          .select("id").single();
        if (insertError) console.error("LP Imobiliária: Erro ao inserir lead:", insertError);
      ...
      supabase.functions.invoke('create-pipedrive-lead', {
        ...
        if (!pipeError && pipeResult?.success) {
          console.log("LP Imobiliária: Pipedrive lead criado (background):", pipeResult.deal_id);
      ...
      }).catch((e) => console.error("LP Imobiliária: Pipedrive exception (background):", e));
      ...
      sessionStorage.setItem("orbit_lp_data", JSON.stringify({
        ...
        oqueFaz: "Imobiliária",
        cargo: "Dono / Sócio de Imobiliária",
        nicheSetor: "Imobiliária / Mercado Imobiliário",
      }));

      navigate("/chat");
    } catch (err) {
      console.error("Failed to save lead from LP Imobiliária:", err);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 animate-fade-in">
          ...
          <div className="relative bg-background border border-border rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <div id="lead-form" className="space-y-4 mb-6">
              ...
              />
              <FormInput name="company" value={formData.company} onChange={handleChange} label="Nome da Imobiliária *" icon={Building2} />
            </div>
          ...
          <div className="lg:grid lg:grid-cols-2 lg:gap-12 lg:items-center">
            <div>
              <span className="inline-flex items-center gap-1.5 bg-primary/15 text-primary px-4 py-1.5 rounded-full text-xs font-bold mb-5 border border-primary/25 relative z-10 uppercase tracking-wider">
                ...
                Orbit para Imobiliárias — Gestão com IA
              </span>
              ...
              <div className="w-full max-w-md space-y-4">
                <div className="bg-card/50 border border-border rounded-2xl p-6 backdrop-blur-sm">
                  <span className="inline-flex items-center gap-1.5 bg-primary/15 text-primary px-3 py-1 rounded-full text-xs font-bold border border-primary/25 uppercase tracking-wider mb-4">
                    ...
                    O que os agentes resolvem na sua imobiliária
                  </span>
                  <div className="space-y-3 mt-2">
                    {[
                      { problem: "Follow-up esquecido", solved: "Cadências automáticas" },
                      { problem: "Equipe sem direção", solved: "Treinamento contínuo" },
                      { problem: "Indicadores inexistentes", solved: "KPIs monitorados 24/7" },
                      { problem: "Processos cada um do seu jeito", solved: "Padrão operacional" },
                    ].map((item) => (
        ...
        <div className="max-w-2xl mx-auto">
          <div className="rounded-[20px] border border-primary/30 bg-card p-6 lg:p-8">
            ...
            <p className="mt-5 text-foreground/90 italic text-base lg:text-lg leading-relaxed">
              "Eu coordeno dezenas de agentes de IA dentro do Orbit. Quando sua imobiliária se conecta, eu analiso o contexto do seu negócio e configuro os agentes ideais para sua operação. Eles estruturam processos, treinam corretores, monitoram KPIs e melhoram o atendimento ao cliente — sem depender só de você."
            </p>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-5 py-8 lg:py-14 lg:px-16">
          <div className="text-center mb-8">
            <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-2">
              O caos invisível de toda <span className="text-primary">imobiliária:</span>
            </h2>
            <p className="text-sm lg:text-base text-muted-foreground max-w-2xl mx-auto">
              Você abriu uma imobiliária para ter autonomia, mas acabou preso em um ciclo infinito de cobranças, retrabalho e follow-up.
            </p>
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 lg:mb-12">
            <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-2">
              ...
              <span className="text-primary">organizar sua imobiliária</span>
            </h2>
        ...
        <div className="max-w-sm lg:max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3 p-3 lg:p-5 rounded-xl bg-primary/10 border border-primary/25">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Onboarding em grupo e sessões de tira-dúvidas semanais para garantir que sua imobiliária tire o máximo do Orbit.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-8 lg:py-16 lg:px-16">
        <div className="max-w-md mx-auto text-center">
          <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-3">
            Pronto para tirar sua imobiliária do{" "}
            <span className="text-primary">caos operacional?</span>
          </h2>
          <p className="text-sm text-muted-foreground mb-6">
            Agende uma demonstração gratuita de 30 minutos. Descubra como dezenas de agentes de IA podem organizar sua imobiliária — sem contratar mais ninguém.
          </p>
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 sm:gap-8 lg:gap-12 mb-8 sm:mb-10 md:mb-12">
            <div className="max-w-sm">
              ...
              <p className="text-muted-foreground text-sm sm:text-base mb-4 sm:mb-6">
                Time de agentes de IA para gestão empresarial. Dezenas de especialistas trabalhando 24/7 para organizar sua imobiliária.
              </p>
            ...
            <div>
              ...
              <ul className="space-y-3 sm:space-y-4">
                <li className="flex items-center gap-2 sm:gap-3">
                  ...
                  <a href="https://wa.me/5548991206282?text=Ol%C3%A1!%20Quero%20conhecer%20o%20Orbit%20para%20minha%20imobili%C3%A1ria" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors text-sm sm:text-base">
                    (48) 99120-6282
                  </a>
                </li>
