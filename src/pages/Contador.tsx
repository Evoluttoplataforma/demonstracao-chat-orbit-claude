...
import { getContadorSessionVariant } from "@/components/chat/contadorCopyVariants";
...
import {
  ...
  Instagram, MapPin, AlertTriangle,
} from "lucide-react";

/* ── FAQ Data (Contadores) ── */
const faqItems = [
  { q: "O que é o Orbit?", a: "O Orbit é uma plataforma que entrega dezenas de agentes de IA especializados para executar a gestão do seu escritório contábil. Cada agente cuida de uma área — processos, pessoas, indicadores, treinamento e mais. Eles trabalham 24/7, integrados aos dados do seu negócio." },
...
/* ── Pain Points (Contadores) ── */
const painPoints = [
  { bold: "Cada obrigação depende de mim para sair certa.", sub: "Se eu não reviso, sai com erro." },
  { bold: "Perco clientes por falta de proatividade,", sub: "não por falta de competência técnica." },
  { bold: "Minha equipe vive apagando incêndio fiscal", sub: "e eu fico preso no operacional." },
  { bold: "Contratei gente boa, mas sem processo", sub: "cada um faz do seu jeito." },
...
const agents = [
  ...
  { icon: FolderCog, name: "Ag. de Processos", desc: "Mapeia processos contábeis, gera instruções de trabalho, playbooks e padrões", equiv: "Analista de processos sênior — R$8-12k/mês" },
  ...
  { icon: GraduationCap, name: "Ag. de Treinamento", desc: "Cria microlearning diário sobre legislação, processos e boas práticas via WhatsApp", equiv: "Coordenador de T&D — R$7-12k/mês" },
  ...
  { icon: Search, name: "Ag. de Pesquisa", desc: "Analisa pesquisas de satisfação, clima organizacional e gera insights acionáveis", equiv: "Analista de dados — R$6-10k/mês" },
  { icon: AlertTriangle, name: "Ag. de Riscos", desc: "Mapeia riscos de prazos fiscais, obrigações acessórias e propõe mitigações", equiv: "Analista de riscos — R$8-12k/mês" },
  { icon: Briefcase, name: "Ag. de Oportunidades", desc: "Mapeia oportunidades de novos serviços, upsell e cross-sell para clientes", equiv: "Analista de inteligência — R$8-12k/mês" },
  ...
  { icon: TrendingUp, name: "Ag. de Vendas", desc: "Estrutura funil comercial, cadências de follow-up e análise de pipeline", equiv: "Coord. comercial — R$8-15k/mês" },
...
/* ── Results ── */
const results = [
  { value: "-40%", label: "Retrabalho", desc: "Agentes de Processos e Documentos reduziram 40% do tempo em refação de obrigações" },
...
const processSteps = [
  ...
  { icon: Brain, number: "02", title: "Seus agentes são configurados", desc: "Com base na análise, a Olívia ativa os agentes ideais para sua operação contábil. Cada um recebe o contexto do seu escritório." },
...
const Contador = () => {
  ...
  const variant = useMemo(() => getContadorSessionVariant(), []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (lookupError) console.error("LP Contador: Lookup error:", lookupError);

      let leadId: string | null = null;
      const resolvedCopyVariant = copyVariant || variant?.id || "CT-A";
      const leadPayload = {
        nome,
        sobrenome,
      ...
      if (existingLead) {
        ...
        if (updateError) console.error("LP Contador: Erro ao atualizar lead:", updateError);
        leadId = existingLead.id;
      } else {
        const { data: inserted, error: insertError } = await supabase.from("leads")
          .insert(leadPayload)
          .select("id").single();
        if (insertError) console.error("LP Contador: Erro ao inserir lead:", insertError);
      ...
      supabase.functions.invoke('create-pipedrive-lead', {
        ...
        if (!pipeError && pipeResult?.success) {
          console.log("LP Contador: Pipedrive lead criado (background):", pipeResult.deal_id);
      ...
      }).catch((e) => console.error("LP Contador: Pipedrive exception (background):", e));
      ...
      sessionStorage.setItem("orbit_lp_data", JSON.stringify({
        ...
        oqueFaz: "Escritório Contábil",
        cargo: "Contador / Sócio",
        nicheSetor: "Contabilidade / Escritório Contábil",
      }));

      navigate("/chat");
    } catch (err) {
      console.error("Failed to save lead from LP Contador:", err);
          ...
          <div className="lg:grid lg:grid-cols-2 lg:gap-12 lg:items-center">
            <div>
              <span className="inline-flex items-center gap-1.5 bg-primary/15 text-primary px-4 py-1.5 rounded-full text-xs font-bold mb-5 border border-primary/25 relative z-10 uppercase tracking-wider">
                ...
                Orbit para Contadores — Gestão com IA
              </span>
              ...
              <div className="w-full max-w-md space-y-4">
                <div className="bg-card/50 border border-border rounded-2xl p-6 backdrop-blur-sm">
                  <span className="inline-flex items-center gap-1.5 bg-primary/15 text-primary px-3 py-1 rounded-full text-xs font-bold border border-primary/25 uppercase tracking-wider mb-4">
                    ...
                    O que os agentes resolvem no seu escritório
                  </span>
                  <div className="space-y-3 mt-2">
                    {[
                      { problem: "Retrabalho em obrigações", solved: "Processos padronizados" },
                      { problem: "Equipe sem direção", solved: "Treinamento contínuo" },
                      { problem: "Indicadores inexistentes", solved: "KPIs monitorados 24/7" },
                      { problem: "Prazos fiscais estourados", solved: "Gestão de riscos ativa" },
                    ].map((item) => (
                      <div key={item.problem} className="flex items-center gap-3">
                        <div className="flex-1">
                          <div className="flex items-center justify-between text-xs mb-1">
                            <span className="text-muted-foreground line-through">{item.problem}</span>
                            <span className="text-primary font-bold">{item.solved}</span>
                          </div>
                          <div className="h-1.5 rounded-full bg-secondary overflow-hidden">
                            <div className="h-full rounded-full bg-gradient-to-r from-primary to-primary/60" style={{ width: "100%" }} />
                          </div>
                        </div>
                      </div>
                    ))}
        ...
        <div className="max-w-2xl mx-auto">
          <div className="rounded-[20px] border border-primary/30 bg-card p-6 lg:p-8">
            ...
            <p className="mt-5 text-foreground/90 italic text-base lg:text-lg leading-relaxed">
              "Eu coordeno dezenas de agentes de IA dentro do Orbit. Quando seu escritório contábil se conecta, eu analiso o contexto do seu negócio e configuro os agentes ideais para sua operação. Eles estruturam processos, treinam equipe, monitoram KPIs e garantem que as obrigações saiam no prazo — sem depender só de você."
            </p>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-5 py-8 lg:py-14 lg:px-16">
          <div className="text-center mb-8">
            <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-2">
              O caos invisível de todo <span className="text-primary">escritório contábil:</span>
            </h2>
            <p className="text-sm lg:text-base text-muted-foreground max-w-2xl mx-auto">
              Você abriu um escritório para ter autonomia, mas acabou preso em um ciclo infinito de prazos, retrabalho e cobrança da equipe.
            </p>
          </div>
      ...
      <section className="px-5 py-8 lg:py-14 lg:px-16">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 lg:mb-12">
            <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-2">
              Conheça o time de IA que vai{" "}
              <span className="text-primary">organizar seu escritório</span>
            </h2>
            <p className="text-sm text-muted-foreground max-w-2xl mx-auto">
              Cada agente é especialista em uma área da gestão. Eles trabalham 24/7 dentro da sua operação — sem férias, sem retrabalho, sem desculpas.
            </p>
          </div>
          ...
          <div className="mt-8 rounded-[20px] border border-primary/30 bg-card p-6 lg:p-8 text-center">
            <p className="text-base lg:text-lg text-foreground/90 leading-relaxed">
              Se você contratasse esse time como CLT, gastaria entre{" "}
              <span className="text-destructive line-through font-bold">R$100.000 e R$200.000/mês</span>.{" "}
              Com o Orbit, tudo isso custa a partir de{" "}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 lg:mb-12">
            <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-2">
              ...
              <span className="text-primary">saiu do caos</span>
            </h2>
          </div>
        ...
        <div className="max-w-sm lg:max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3 p-3 lg:p-5 rounded-xl bg-primary/10 border border-primary/25">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Onboarding em grupo e sessões de tira-dúvidas semanais para garantir que seu escritório tire o máximo do Orbit.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-8 lg:py-16 lg:px-16">
        <div className="max-w-md mx-auto text-center">
          <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-3">
            Pronto para tirar seu escritório do{" "}
            <span className="text-primary">caos operacional?</span>
          </h2>
          <p className="text-sm text-muted-foreground mb-6">
            Agende uma demonstração gratuita de 30 minutos. Descubra como dezenas de agentes de IA podem organizar seu escritório contábil — sem contratar mais ninguém.
          </p>
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 sm:gap-8 lg:gap-12 mb-8 sm:mb-10 md:mb-12">
            <div className="max-w-sm">
              ...
              <p className="text-muted-foreground text-sm sm:text-base mb-4 sm:mb-6">
                Time de agentes de IA para gestão empresarial. Dezenas de especialistas trabalhando 24/7 para organizar seu escritório contábil.
              </p>
            ...
            <div>
              ...
              <ul className="space-y-3 sm:space-y-4">
                <li className="flex items-center gap-2 sm:gap-3">
                  ...
                  <a href="https://wa.me/5548991206282?text=Ol%C3%A1!%20Quero%20conhecer%20o%20Orbit%20para%20meu%20escrit%C3%B3rio%20cont%C3%A1bil" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors text-sm sm:text-base">
                    (48) 99120-6282
                  </a>
                </li>
