...
import { getEngenhariaSessionVariant } from "@/components/chat/engenhariaCopyVariants";
...
import {
  ...
  Instagram, MapPin, AlertTriangle, HardHat, Ruler,
} from "lucide-react";

/* ── FAQ Data ── */
const faqItems = [
  { q: "O que é o Orbit?", a: "O Orbit é uma plataforma que entrega dezenas de agentes de IA especializados para executar a gestão da sua empresa de engenharia. Cada agente cuida de uma área — processos, projetos, indicadores, treinamento e mais. Eles trabalham 24/7, integrados aos dados do seu negócio." },
  { q: "Funciona para empresas pequenas de engenharia?", a: "Sim. Funciona para empresas de 3 a 200 pessoas. Na verdade, quanto menor a equipe, mais impacto os agentes geram — porque substituem funções que você não consegue contratar." },
  ...
  { q: "A demonstração é gratuita?", a: "Sim. São 30 minutos para você conhecer seus agentes, entender como atuariam na sua empresa e ver resultados reais. Sem compromisso." },
  { q: "E se minha empresa já usa ERP ou software de projetos?", a: "Perfeito. O Orbit não substitui suas ferramentas — ele preenche a lacuna entre ter a ferramenta e realmente usar. Os agentes garantem que processos sejam seguidos, indicadores monitorados e equipe treinada." },
];

/* ── Pain Points ── */
const painPoints = [
  { bold: "Cada obra depende de mim para não sair do trilho.", sub: "Se eu não acompanho, estoura prazo e orçamento." },
  { bold: "Perco margem por falta de processo,", sub: "não por falta de competência técnica." },
  { bold: "Minha equipe vive apagando incêndio", sub: "e eu fico preso no operacional." },
  { bold: "Contratei engenheiros bons, mas sem gestão", sub: "cada um executa do seu jeito." },
  { bold: "Não consigo tirar férias", sub: "porque nada funciona sem mim." },
  { bold: "Já tentei planilha, ERP, consultor…", sub: "e nada se mantém funcionando." },
];

/* ── AI Agents ── */
const agents = [
  { icon: Target, name: "Ag. Estrategista", desc: "Constrói planejamento estratégico, SWOT, BSC, objetivos e planos de ação para a empresa", equiv: "Consultor de planejamento — R$30-80k/projeto" },
  { icon: FolderCog, name: "Ag. de Processos", desc: "Mapeia processos de obra, escritório e suprimentos, gera instruções de trabalho e playbooks", equiv: "Analista de processos sênior — R$8-12k/mês" },
  { icon: Ruler, name: "Ag. de Projetos", desc: "Gerencia cronogramas, dependências, marcos de entrega e desvios de escopo", equiv: "Gerente de projetos — R$10-18k/mês" },
  ...
  { icon: GraduationCap, name: "Ag. de Treinamento", desc: "Cria microlearning diário sobre segurança, normas técnicas e boas práticas via WhatsApp", equiv: "Coordenador de T&D — R$7-12k/mês" },
  { icon: BarChart3, name: "Ag. de Indicadores", desc: "Monitora KPIs de obra, produtividade, custos, gera hipóteses de causa raiz", equiv: "Analista de BI — R$8-15k/mês" },
  { icon: Search, name: "Ag. de Pesquisa", desc: "Analisa pesquisas de satisfação, clima organizacional e gera insights", equiv: "Analista de dados — R$6-10k/mês" },
  { icon: AlertTriangle, name: "Ag. de Riscos", desc: "Mapeia riscos de projeto, prazos, segurança e propõe mitigações", equiv: "Analista de riscos — R$8-12k/mês" },
  { icon: Briefcase, name: "Ag. de Compras", desc: "Otimiza cadeia de suprimentos, cotações, aprovações e rastreabilidade", equiv: "Analista de suprimentos — R$7-12k/mês" },
  ...
  { icon: FileText, name: "Ag. de Documentos", desc: "Gera documentos técnicos, relatórios e checklists estruturados", equiv: "Analista documental — R$5-8k/mês" },
...
/* ── Results ── */
const results = [
  { value: "-40%", label: "Retrabalho", desc: "Agentes de Processos e Projetos reduziram 40% do tempo em refação e conferência" },
...
const processSteps = [
  ...
  { icon: Brain, number: "02", title: "Seus agentes são configurados", desc: "Com base na análise, a Olívia ativa os agentes ideais para sua operação de engenharia." },
...
const Engenharia = () => {
  ...
  const variant = useMemo(() => getEngenhariaSessionVariant(), []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (lookupError) console.error("LP Engenharia: Lookup error:", lookupError);

      let leadId: string | null = null;
      const resolvedCopyVariant = copyVariant || variant?.id || "EN-A";
      const leadPayload = {
        nome,
        sobrenome,
        ...
        cargo: "Dono / Sócio de Engenharia",
        oque_faz: "Engenharia",
        ...cleanUtm,
      } as any;
      ...
      if (existingLead) {
        ...
        if (updateError) console.error("LP Engenharia: Erro ao atualizar lead:", updateError);
        leadId = existingLead.id;
      } else {
        const { data: inserted, error: insertError } = await supabase.from("leads")
          .insert(leadPayload)
          .select("id").single();
        if (insertError) console.error("LP Engenharia: Erro ao inserir lead:", insertError);
      ...
      supabase.functions.invoke('create-pipedrive-lead', {
        ...
        if (!pipeError && pipeResult?.success) {
          console.log("LP Engenharia: Pipedrive lead criado (background):", pipeResult.deal_id);
      ...
      }).catch((e) => console.error("LP Engenharia: Pipedrive exception (background):", e));
      ...
      sessionStorage.setItem("orbit_lp_data", JSON.stringify({
        ...
        oqueFaz: "Engenharia",
        cargo: "Dono / Sócio de Engenharia",
        nicheSetor: "Engenharia / Construção",
      }));

      navigate("/chat");
    } catch (err) {
      console.error("Failed to save lead from LP Engenharia:", err);
          ...
          <div className="lg:grid lg:grid-cols-2 lg:gap-12 lg:items-center">
            <div>
              <span className="inline-flex items-center gap-1.5 bg-primary/15 text-primary px-4 py-1.5 rounded-full text-xs font-bold mb-5 border border-primary/25 relative z-10 uppercase tracking-wider">
                ...
                Orbit para Engenharia — Gestão com IA
              </span>
              ...
              <div className="w-full max-w-md space-y-4">
                <div className="bg-card/50 border border-border rounded-2xl p-6 backdrop-blur-sm">
                  <span className="inline-flex items-center gap-1.5 bg-primary/15 text-primary px-3 py-1 rounded-full text-xs font-bold border border-primary/25 uppercase tracking-wider mb-4">
                    ...
                    O que os agentes resolvem na sua empresa
                  </span>
                  <div className="space-y-3 mt-2">
                    {[
                      { problem: "Obra sem cronograma claro", solved: "Projetos gerenciados" },
                      { problem: "Equipe sem padrão", solved: "Treinamento contínuo" },
                      { problem: "Indicadores inexistentes", solved: "KPIs monitorados 24/7" },
                      { problem: "Orçamento estourado", solved: "Custos controlados" },
                    ].map((item) => (
        ...
        <div className="max-w-2xl mx-auto">
          <div className="rounded-[20px] border border-primary/30 bg-card p-6 lg:p-8">
            ...
            <p className="mt-5 text-foreground/90 italic text-base lg:text-lg leading-relaxed">
              "Eu coordeno dezenas de agentes de IA dentro do Orbit. Quando sua empresa de engenharia se conecta, eu analiso o contexto do seu negócio e configuro os agentes ideais para sua operação. Eles estruturam processos, gerenciam projetos, treinam equipe e monitoram KPIs — sem depender só de você."
            </p>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-5 py-8 lg:py-14 lg:px-16">
          <div className="text-center mb-8">
            <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-2">
              O caos invisível de toda <span className="text-primary">empresa de engenharia:</span>
            </h2>
            <p className="text-sm lg:text-base text-muted-foreground max-w-2xl mx-auto">
              Você abriu uma empresa para ter autonomia, mas acabou preso em um ciclo infinito de obras, prazos e retrabalho.
            </p>
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 lg:mb-12">
            <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-2">
              ...
              <span className="text-primary">organizar sua empresa</span>
            </h2>
        ...
        <div className="max-w-sm lg:max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3 p-3 lg:p-5 rounded-xl bg-primary/10 border border-primary/25">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Onboarding em grupo e sessões de tira-dúvidas semanais para garantir que sua empresa tire o máximo do Orbit.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-8 lg:py-16 lg:px-16">
        <div className="max-w-md mx-auto text-center">
          <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-3">
            Pronto para tirar sua empresa do{" "}
            <span className="text-primary">caos operacional?</span>
          </h2>
          <p className="text-sm text-muted-foreground mb-6">
            Agende uma demonstração gratuita de 30 minutos. Descubra como dezenas de agentes de IA podem organizar sua empresa de engenharia — sem contratar mais ninguém.
          </p>
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 sm:gap-8 lg:gap-12 mb-8 sm:mb-10 md:mb-12">
            <div className="max-w-sm">
              ...
              <p className="text-muted-foreground text-sm sm:text-base mb-4 sm:mb-6">
                Time de agentes de IA para gestão empresarial. Dezenas de especialistas trabalhando 24/7 para organizar sua empresa.
              </p>
            ...
            <div>
              ...
              <ul className="space-y-3 sm:space-y-4">
                <li className="flex items-center gap-2 sm:gap-3">
                  ...
                  <a href="https://wa.me/5548991206282?text=Ol%C3%A1!%20Quero%20conhecer%20o%20Orbit%20para%20minha%20empresa%20de%20engenharia" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors text-sm sm:text-base">
                    (48) 99120-6282
                  </a>
                </li>
