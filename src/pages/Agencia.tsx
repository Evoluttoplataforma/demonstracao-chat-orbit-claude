...
import { getAgenciaSessionVariant } from "@/components/chat/agenciaCopyVariants";
...
/* ── FAQ Data (Agências) ── */
const faqItems = [
  { q: "O que é o Orbit?", a: "O Orbit é uma plataforma que entrega dezenas de agentes de IA especializados para executar a gestão da sua agência. Cada agente cuida de uma área — processos, pessoas, indicadores, treinamento e mais. Eles trabalham 24/7, integrados aos dados do seu negócio." },
  { q: "Funciona para agências pequenas?", a: "Sim. Funciona para agências de 3 a 200 pessoas. Na verdade, quanto menor a equipe, mais impacto os agentes geram — porque substituem funções que você não consegue contratar." },
  { q: "Quanto tempo leva para implementar?", a: "Em até 7 dias seu time de agentes está operando. A Olívia, nossa especialista em IA, analisa o contexto da sua agência e configura os agentes ideais para sua operação." },
  ...
  { q: "A demonstração é gratuita?", a: "Sim. São 30 minutos para você conhecer seus agentes, entender como atuariam na sua agência e ver resultados reais. Sem compromisso." },
  { q: "E se minha agência já usa ferramentas de gestão?", a: "Perfeito. O Orbit não substitui suas ferramentas — ele preenche a lacuna entre ter a ferramenta e realmente usar. Os agentes garantem que processos sejam seguidos, indicadores monitorados e equipe treinada." },
];

/* ── Pain Points (Agências) ── */
const painPoints = [
  { bold: "Cada entrega depende de mim para sair certa.", sub: "Se eu não reviso, sai errado." },
  { bold: "Perco clientes por falta de processo,", sub: "não por falta de talento." },
  { bold: "Minha equipe vive apagando incêndio", sub: "e eu fico preso no operacional." },
  { bold: "Contratei gente boa, mas sem processo", sub: "todo mundo faz do seu jeito." },
...
const agents = [
  ...
  { icon: AlertTriangle, name: "Ag. de Riscos", desc: "Mapeia riscos de projeto, prazos e entregas, propõe mitigações", equiv: "Analista de riscos — R$8-12k/mês" },
...
/* ── Results ── */
const results = [
  { value: "-40%", label: "Retrabalho", desc: "Agentes de Processos e Documentos reduziram 40% do tempo em refação de entregas" },
...
/* ── Process Steps ── */
const processSteps = [
  { icon: Zap, number: "01", title: "Conecte sua agência", desc: "Cadastre sua agência em menos de 5 minutos. A Olívia analisa o contexto do seu negócio." },
  { icon: Brain, number: "02", title: "Seus agentes são configurados", desc: "Com base na análise, a Olívia ativa os agentes ideais para sua operação. Cada um recebe o contexto da sua agência." },
...
const Agencia = () => {
  ...
  const variant = useMemo(() => getAgenciaSessionVariant(), []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (lookupError) console.error("LP Agência: Lookup error:", lookupError);

      let leadId: string | null = null;
      const resolvedCopyVariant = copyVariant || variant?.id || "AG-A";
      const leadPayload = {
        nome,
        sobrenome,
      ...
      if (existingLead) {
        ...
        if (updateError) console.error("LP Agência: Erro ao atualizar lead:", updateError);
        leadId = existingLead.id;
      } else {
        const { data: inserted, error: insertError } = await supabase.from("leads")
          .insert(leadPayload)
          .select("id").single();
        if (insertError) console.error("LP Agência: Erro ao inserir lead:", insertError);
      ...
      supabase.functions.invoke('create-pipedrive-lead', {
        ...
        if (!pipeError && pipeResult?.success) {
          console.log("LP Agência: Pipedrive lead criado (background):", pipeResult.deal_id);
      ...
      }).catch((e) => console.error("LP Agência: Pipedrive exception (background):", e));
      ...
      sessionStorage.setItem("orbit_lp_data", JSON.stringify({
        ...
        oqueFaz: "Agência",
        cargo: "CEO / Diretor",
        nicheSetor: "Agência de Marketing / Publicidade",
      }));

      navigate("/chat");
    } catch (err) {
      console.error("Failed to save lead from LP Agência:", err);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 animate-fade-in">
          ...
          <div className="relative bg-background border border-border rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <div id="lead-form" className="space-y-4 mb-6">
              ...
              />
              <FormInput name="company" value={formData.company} onChange={handleChange} label="Nome da Agência *" icon={Building2} />
            </div>
          ...
          <div className="lg:grid lg:grid-cols-2 lg:gap-12 lg:items-center">
            <div>
              <span className="inline-flex items-center gap-1.5 bg-primary/15 text-primary px-4 py-1.5 rounded-full text-xs font-bold mb-5 border border-primary/25 relative z-10 uppercase tracking-wider">
                ...
                Orbit para Agências — Gestão com IA
              </span>
              ...
              <div className="w-full max-w-md space-y-4">
                <div className="bg-card/50 border border-border rounded-2xl p-6 backdrop-blur-sm">
                  <span className="inline-flex items-center gap-1.5 bg-primary/15 text-primary px-3 py-1 rounded-full text-xs font-bold border border-primary/25 uppercase tracking-wider mb-4">
                    ...
                    O que os agentes resolvem na sua agência
                  </span>
                  <div className="space-y-3 mt-2">
                    {[
                      { problem: "Retrabalho em entregas", solved: "Processos padronizados" },
                      { problem: "Equipe sem direção", solved: "Treinamento contínuo" },
                      { problem: "Indicadores inexistentes", solved: "KPIs monitorados 24/7" },
                      { problem: "Prazos estourados", solved: "Gestão de riscos ativa" },
                    ].map((item) => (
        ...
        <div className="max-w-2xl mx-auto">
          <div className="rounded-[20px] border border-primary/30 bg-card p-6 lg:p-8">
            ...
            <p className="mt-5 text-foreground/90 italic text-base lg:text-lg leading-relaxed">
              "Eu coordeno dezenas de agentes de IA dentro do Orbit. Quando sua agência se conecta, eu analiso o contexto do seu negócio e configuro os agentes ideais para sua operação. Eles estruturam processos, treinam equipe, monitoram KPIs e garantem que as entregas saiam no prazo — sem depender só de você."
            </p>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-5 py-8 lg:py-14 lg:px-16">
          <div className="text-center mb-8">
            <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-2">
              O caos invisível de toda <span className="text-primary">agência:</span>
            </h2>
            <p className="text-sm lg:text-base text-muted-foreground max-w-2xl mx-auto">
              Você montou uma agência para ter liberdade, mas acabou preso em um ciclo infinito de apagar incêndios, refazer entregas e cobrar a equipe.
            </p>
          </div>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-8 lg:mb-12">
            <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-2">
              ...
              <span className="text-primary">organizar sua agência</span>
            </h2>
        ...
        <div className="max-w-sm lg:max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3 p-3 lg:p-5 rounded-xl bg-primary/10 border border-primary/25">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Onboarding em grupo e sessões de tira-dúvidas semanais para garantir que sua agência tire o máximo do Orbit.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-8 lg:py-16 lg:px-16">
        <div className="max-w-md mx-auto text-center">
          <h2 className="text-xl lg:text-3xl font-extrabold text-foreground uppercase tracking-tight mb-3">
            Pronto para tirar sua agência do{" "}
            <span className="text-primary">caos operacional?</span>
          </h2>
          <p className="text-sm text-muted-foreground mb-6">
            Agende uma demonstração gratuita de 30 minutos. Descubra como dezenas de agentes de IA podem organizar sua agência — sem contratar mais ninguém.
          </p>
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6 sm:gap-8 lg:gap-12 mb-8 sm:mb-10 md:mb-12">
            <div className="max-w-sm">
              ...
              <p className="text-muted-foreground text-sm sm:text-base mb-4 sm:mb-6">
                Time de agentes de IA para gestão empresarial. Dezenas de especialistas trabalhando 24/7 para organizar sua agência.
              </p>
            ...
            <div>
              ...
              <ul className="space-y-3 sm:space-y-4">
                <li className="flex items-center gap-2 sm:gap-3">
                  ...
                  <a href="https://wa.me/5548991206282?text=Ol%C3%A1!%20Quero%20conhecer%20o%20Orbit%20para%20minha%20ag%C3%AAncia" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors text-sm sm:text-base">
                    (48) 99120-6282
                  </a>
                </li>
